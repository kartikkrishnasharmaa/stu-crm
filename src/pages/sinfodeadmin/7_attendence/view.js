...
function ViewAttendance() {
  ...
  const fetchAttendance = async () => {
    if (!selectedBranch || !month) return alert("Select month & branch first");
    ...
    try {
      ...
      const res = await axios.get("/attendance/monthly", {
        headers: { Authorization: `Bearer ${token}` },
        params: { month, branch_id: parseInt(selectedBranch) },
      });
      ...
      {Object.values(grouped).length > 0 ? (
        ...
        !loading && (
          <p className="text-center text-gray-500 mt-6">
            🚫 No attendance data found for this month & branch
          </p>
        )
      )}
                ...
                {calendarData.map((day, index) => (
                  <div 
                    ...
                    className={`min-h-12 rounded-lg flex flex-col items-center justify-center p-1
                      ...
                      ${day.status === 'Present' ? 'bg-green-700 text-white font-bold border border-green-300' : ''}
