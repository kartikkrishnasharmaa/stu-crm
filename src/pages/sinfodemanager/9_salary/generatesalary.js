...
const StaffSalaryManagement = () => {
  ...
  // Fetch staff - only for the user's branch
  const fetchStaff = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(`/staff?branch_id=${userBranchId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const generateSalarySlip = () => {
    ...
    try {
      ...
      const branchName = userBranch ? userBranch.branch_name : userBranchId;
      
      doc.text(`Branch: ${branchName}`, 110, 72);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md mb-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            {/* Display current branch instead of selection */}
            {/* <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Branch</label>
              <div className="p-2 bg-gray-100 rounded-md">
                {branches.find(b => b.id == userBranchId)?.branch_name || userBranchId}
              </div>
            </div>
             */}
            <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        ...
                        <p><span className="text-gray-600">Branch:</span> {branches.find(b => b.id == userBranchId)?.branch_name || userBranchId}</p>
                      </div>
                    </div>
