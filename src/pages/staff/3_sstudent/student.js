...
export default function Allstudents() {
  ...
  // Fetch data for dropdowns (branches, courses, batches)
  useEffect(() => {
    async function fetchDropdowns() {
      const token = localStorage.getItem("token");
      try {
        const [branchRes, courseRes, batchRes] = await Promise.all([
          axios.get("/branches/show", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("/courses/show", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("/batches/show", { headers: { Authorization: `Bearer ${token}` } }),
        ]);
  ...
  const filteredStudents = students
    .filter((student) => {
      ...
      // Course
      const matchesCourse =
        !selectedCourse || student.course?.id === parseInt(selectedCourse);
      // Batch
      const matchesBatch =
        !selectedBatch || student.batch?.id === parseInt(selectedBatch);
      // Status
      const matchesStatus =
        !selectedStatus || student.enrollment_status === selectedStatus;
      // Admission date
      const matchesDateFrom =
        !dateFrom || new Date(student.admission_date) >= new Date(dateFrom);
      const matchesDateTo =
        !dateTo || new Date(student.admission_date) <= new Date(dateTo);
      // Search
      const matchesSearch =
  ...
  // STATUS Dropdown Data (for filtering)
  const statusOptions = [
    "Active", "Inactive", "Pending", "Completed", "Withdrawn"
  ];
      ...
      <div className="px-5">
        ...
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-[34px] font-nunito">
            Students <span className="text-[34px] font-nunito">({filteredStudents.length})</span>
          </h1>
        ...
        {/* FILTER BOX */}
        <div className="bg-white mb-3 px-4 py-4 rounded-xl shadow flex flex-wrap gap-4 items-center">
          {/* Branch */}
          {userRole === "admin" && (
            <select
              value={selectedBranch}
              onChange={(e) => setSelectedBranch(e.target.value)}
              className="border p-2 rounded w-auto md:w-44"
            >
          ...
          <select
            ...
            className="border p-2 rounded w-auto md:w-44"
          >
            <option value="latest">Sort: Latest Admission</option>
            <option value="oldest">Sort: Oldest Admission</option>
          ...
          <input
            ...
            className="border p-2 rounded w-auto md:w-60"
          />
        </div>
